...
btCylinderShape* createCylinderCollider( const Ogre::MovableObject* mo )
{
  ...
  return shape;
    ...
    public:
      ...
      const Ogre::Vector3* getVertices( ) { return mVertexBuffer; }
    ...
    private:
      ...
      typedef std::map<unsigned char, std::vector<Ogre::Vector3>*> BoneIndex;
...
public:
  ...
  virtual ~CollisionObject( )
    ...
    DynamicsWorld::DynamicsWorld( const Ogre::Vector3& gravity ) : CollisionWorld( NULL ) // prevent CollisionWorld from creating a world
    {
    ...
    static btCollisionShape* getCollisionShape( Ogre::Entity* ent, ColliderType ct )
    {
      if ( ent->hasSkeleton( ) )
      {
    ...
    btRigidBody* DynamicsWorld::addRigidBody( float mass, Ogre::Entity* ent, ColliderType ct, CollisionListener* listener,
                                              int group, int mask )
    {
    ...
    btCollisionObject* CollisionWorld::addCollisionObject( Ogre::Entity* ent, ColliderType ct, int group, int mask )
    {
      auto node = ent->getParentSceneNode( );
      OgreAssert( node, "entity must be attached" );
    ...
    void CollisionWorld::rayTest( const Ogre::Ray& ray, RayResultCallback* callback, float maxDist )
    {
...
//------------------------------------------------------------------------------------------------
    VertexIndexToShape::VertexIndexToShape( const Ogre::Affine3& transform )
      : mVertexBuffer( 0 ), mIndexBuffer( 0 ), mVertexCount( 0 ), mIndexCount( 0 ), mBounds( Ogre::Vector3( -1, -1, -1 ) ),
        mBoundRadius( -1 ), mBoneIndex( 0 ), mTransform( transform ), mScale( 1 )
    {
    }
//------------------------------------------------------------------------------------------------
    VertexIndexToShape::VertexIndexToShape( const Ogre::Entity* entity, const Ogre::Affine3& transform ) : VertexIndexToShape( transform )
    {
      addEntity( entity, transform );
    }
//------------------------------------------------------------------------------------------------
    VertexIndexToShape::VertexIndexToShape( Ogre::Renderable* rend, const Ogre::Affine3& transform ) : VertexIndexToShape( transform )
    {
...
//------------------------------------------------------------------------------------------------
    void VertexIndexToShape::addEntity( const Ogre::Entity* entity, const Ogre::Affine3& transform )
    {
      ...
      {
        Ogre::SubMesh* sub_mesh = entity->getSubEntity( i )->getSubMesh( );
...
//------------------------------------------------------------------------------------------------
    void VertexIndexToShape::addMesh( const Ogre::MeshPtr& mesh, const Ogre::Affine3& transform )
    {
      // Each entity added need to reset size and radius
      // next time getRadius and getSize are asked, they're computed.
      mBounds = Ogre::Vector3( -1, -1, -1 );
